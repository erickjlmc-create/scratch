...
def fetch_bybit_klines(symbol, interval_str, limit):
    """
    Descarga velas históricas directamente desde la API pública de Bybit v5.
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("retCode") == 0:
                ...
                raw_list.reverse() # Cronológico: más antiguo a más reciente
                candles = []
        ...
        print(f"Error conectando a Bybit para {symbol}: {e}")
    return []
...
def fetch_klines(symbol, interval, limit):
    if interval == "15":
        ...
        bybit_interval = "D"
    return fetch_bybit_klines(symbol, bybit_interval, limit)
...
def fetch_ticker(symbol):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if data.get("retCode") == 0:
                list_item = data["result"]["list"]
                if list_item:
                    return {"lastPrice": str(list_item[0]["lastPrice"])}
    except Exception as e:
        print(f"Error en ticker Bybit para {symbol}: {e}")
    return {"lastPrice": "0"}
...
def analyze_pair(pair):
    ...
    if not c15 or not c4h:
        raise ValueError(f"No se pudieron obtener datos de Bybit para {sym}")
    ...
    rvol_val = (vols[n]/vol_sma[n]) if (vol_sma and vol_sma[n]) else None
    es_fin_de_semana = datetime.now(timezone.utc).weekday() >= 5
    umbral_rvol = 3.0 if es_fin_de_semana else 4.0
    rvol_ok = rvol_val is not None and rvol_val >= umbral_rvol

    ema200a=ema(closes,CFG["EMA200"]); ema200_val=ema200a[n] or closes[n]
    ema100a=ema(closes,CFG["EMA100"]); ema100_val=ema100a[n] or closes[n]
    ...
    ema200_4h_arr = ema(closes4h,CFG["EMA200_4H"])
    ema50_4h_val  = ema50_4h_arr[-1]  if ema50_4h_arr  else closes4h[-1]
    ema200_4h_val = ema200_4h_arr[-1] if ema200_4h_arr else closes4h[-1]
    ...
    if b_l or b_s:
        try:
            c1h = fetch_klines(pair["symbol"], "60", 60)
            if len(c1h) >= 30:
...
def obtener_estado_heatmap(symbol):
    ...
    try:
        ...
        var = sum((v - mean) ** 2 for v in ventana) / largo
        std = var ** 0.5
        if std == 0:
            return "normal"
...
def main():
    ...
    for ev in MACRO_EVENTS:
        ...
        if 0 < mins_to_event <= 15 and not state.get(ev_key):
            notify(
                ...
                f"⚠️ {ev['name']}\n🚫 No abrir nuevas posiciones\n🛡️ Si tienes trade abierto: mueve SL a breakeven",
    ...
    if is_weekly_window and not state.get("_weekly_sent"):
        ...
        for pair in PAIRS:
            try:
                ...
                weekly_lines.append(f"{st} {pair['name']:<12} 4H:{e4h} STC:{stc}{d['stc']:>3} ADX:{d['adx']:>3}")
            except Exception:
    ...
    def btc_line(direction):
        ...
        if direction == "long" and btc["weak"]:
            return f"⚠️ CONTEXTO BTC: Bajando {btc['chg1h']}% en 1H · Reducir tamaño 60%\n"
        if direction == "short" and btc["strong"]:
            return f"⚠️ CONTEXTO BTC: Subiendo {btc['chg1h']}% en 1H · Reducir tamaño 60%\n"
        trend = "ALCISTA" if btc["bull"] else "BAJISTA"
        return f"✅ BTC {trend} ({btc['chg1h']:+.1f}% 1H) · Alineado\n"

    def checklist(grade, direction):
        if grade == "App": return "📋 CHECKLIST: A++ → EJECUTAR DIRECTO"
        if grade == "Ap":  return "📋 CHECKLIST: A+ → EJECUTAR (confirmar 1H alineado)"
        if grade == "B":   return "📋 CHECKLIST B → verificar velas y volumen"
        if grade == "prec": return "📋 CHECKLIST PREC → 50% tamaño"
        return ""
    ...
    for pair in PAIRS:
        ...
        if vol_extrema_ahora and not state.get(vol_key, False):
            notify(
                f"💥 Volatilidad extrema · {pair['name']}",
                f"📶 RVOL: {data['rvol']} | Heatmap: {heatmap_zone}",
                priority="default", tags="fire,warning", destino="radar",
            )
            alerts_sent += 1
        ...
        if trade_activo:
            ...
            if not trade_activo.get("tp1_hit") and trade_activo.get("tp1"):
                if (direccion=="long" and data["price"] >= trade_activo["tp1"]) or (direccion=="short" and data["price"] <= trade_activo["tp1"]):
                    notify(f"🛡️ TP1 Alcanzado · {pair['name']}", "SL a Breakeven", priority="high", tags="shield")
        ...
        def pos_block():
            if not pos: return ""
            return f"💵 Capital: $100 · Riesgo 1% = ${pos['risk_usd']}\n📏 Distancia SL: ${pos['dist']} ({pos['dist_pct']}%)\n📦 Contratos: {pos['contracts']}\n"
        ...
        if grade in ("App","Ap","B","prec") and grade != prev.get("grade"):
            ...
            register_trade()
            notify(
                f"{gl[grade]} {dir_icon} {dir_sym} · {pair['name']}",
                f"🎯 {p}/3 pilares · Precio: ${data['price']}\n🛑 SL: ${data['sl']}\n🎯 TP1: ${data['tp1']}\n{pos_block()}{btc_line(data['direction'])}",
                priority=pri, tags="rotating_light",
            )
            alerts_sent += 1
            new_state[trade_key] = {
                "entry": data["price"], "sl": data["sl"],
                "tp1": data["tp1"], "tp2": data["tp2"], "tp3": data["tp3"],
                "direction": data["direction"], "tp1_hit": False, "tp2_hit": False, "tp3_hit": False,
            }

    save_state({**state, **new_state})
    print(f"Ciclo ejecutado con éxito en Bybit. Alertas enviadas: {alerts_sent}")
